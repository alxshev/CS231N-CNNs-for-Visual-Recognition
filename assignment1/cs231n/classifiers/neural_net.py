...
class TwoLayerNet(object):
	...
	def loss(self, X, y=None, reg=0.0):
		...
		S1 = X @ W1 + b1 # [N x H]
		S1ReLU = np.maximum(0, S1) # [N x H]
		S2 = S1ReLU @ W2 + b2 # [N x C]
		scores = S2 # [N x C]
		...
		expS2 = np.exp(S2)
		softmaxS2 = expS2 / expS2.sum(axis=1, keepdims=True) # [N x C]
		loss = -np.log(softmaxS2[(range(N), y)]).sum() / N + reg * (L2(W1) + L2(W2))
		# *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
		...
		dS2[range(N), y] -= 1
		dS2 /= N
		dW2 = S1ReLU.T @ dS2 + 2 * reg * W2 # [H x C]

		# Backprop on layer 1
		dS1 = dS2 @ W2.T # [	]
		dS1[S1 < 0] = 0 # apply reLU
		dW1 = X.T @ dS1  + 2 * reg * W1
