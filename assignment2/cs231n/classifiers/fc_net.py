...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        data_loss, softmax_grad = softmax_loss(out2, y)
        reg_loss = 0.5 * reg * ((W1 ** 2).sum() + (W2 ** 2).sum())
        loss = data_loss + reg_loss

        dScores2, dW2_data, db2_data = affine_backward(softmax_grad, cache2)
        dx_data, dW1_data, db1_data = affine_relu_backward(dScores2, cache1)

        grads['W1'] = dW1_data + reg * W1
        grads['W2'] = dW2_data + reg * W2
        grads['b1'] = db1_data
        grads['b2'] = db2_data
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for l in range(1, L):
            ...
            if normalization in ["batchnorm", "layernorm"]:
                self.params[f"gamma{l}"] = np.ones(current_layer_dims)
                self.params[f"beta{l}"] = np.zeros(current_layer_dims)
    ...
    def loss(self, X, y=None):
        ...
        for l in range(1, self.num_layers):
            if self.normalization == "batchnorm":
                W, b, gamma, beta = [self.params[x + str(l)]  for x in ["W", "b", "gamma", "beta"]]
